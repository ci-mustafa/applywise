...
class Application(models.Model):
    ...
    - slug: A URL-friendly identifier for the application,
      unique for each entry.
    - position_level: The level of the position (Junior, Associate,
      Mid senior, Senior).
    ...
    - status: The current status of the application (Applied, Interview,
      Offer, Rejected).
    - applied_method: The method used for applying (Website, Job board,
      Referral, Email).
    ...
    - follow_up_date: The date when the user plans to
      follow up on the application.
    - created_on: The timestamp for when the application
      was created (automatically set).
    - applicant: A foreign key linking to the User model,
      indicating which user submitted the application.
    Meta:
    - Ordering: Applications are ordered by the creation date,
    ...
    applied_method = models.CharField(max_length=9,
                                      choices=APPLIED_METHOD_CHOICES,
                                      default="website")
